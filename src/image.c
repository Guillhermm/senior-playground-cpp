...
static RawImage *imageOpen(char *fileName) {
    RawImage     *image;
    unsigned long *rowStart, *rowSize, ulTmp;
    int            x, i;

    image = (RawImage *)malloc(sizeof(RawImage));
    if (image == NULL) {
...
static void imageClose(RawImage *image) {
    int i;
    fclose(image->file);
    for (i = 0; i <= image->sizeZ; i++)
        free(image->tmp[i]);
...
static void imageGetRow(RawImage *image, unsigned char *buf, int y, int z) {
    ...
    if ((image->type & 0xFF00) == 0x0100) { /* RLE image */
        ...
        fread(image->tmp[0], 1,
              (unsigned int)image->rowSize[y + z * image->sizeY],
              image->file);
...
IMAGE *ImageLoad(char *fileName) {
    ...
    result->imagic = image->imagic;
    result->type   = image->type;
    result->dim    = image->dim;
    result->sizeX  = image->sizeX;
    result->sizeY  = image->sizeY;
    result->sizeZ  = image->sizeZ;

    /* Round up so rows are long-word aligned */
    sx = ((image->sizeX) * (image->sizeZ) + 3) >> 2;

    result->data = (unsigned char *)malloc(sx * image->sizeY * sizeof(unsigned int));
    if (result->data == NULL) {
        fprintf(stderr, "Out of memory!\n");
        exit(EXIT_FAILURE);
    }

    imageGetRawData(image, (char *)result->data);
    imageClose(image);
    return result;
}
